...
volatile uint8_t FLAG_ZERO_TX = 0; /* NODE ZERO WILL NOT TRANSMIT IF THIS FALG = 0 SOMETHING LIKE ACK */
volatile uint8_t FLAG_ONE_TX = 0; /* NODE ONE WILL NOT TRANSMIT IF THIS FALG = 0 SOMETHING LIKE ACK */
volatile uint8_t FLAG_TWO_TX = 0; /* NODE TWO WILL NOT TRANSMIT IF THIS FALG = 0 SOMETHING LIKE ACK */
...
void CANIntHandler(void)
{
    ...
    //
    // Check if the cause is message object 3. MAILBOX 2
    //
...
void vTaskCode_NODE_ZERO_TX(void * pvParameters)
{
    ...
    while (1)
    {
        ...
        if (FLAG_ZERO_TX == 1) /* check if this flag == 1 so the receiver received succesfuly and you can transmit */
        {
...
void vTaskCode_NODE_ONE_TX(void * pvParameters)
{
    ...
    pui8MsgData = (uint8_t *) &ui32MsgData;
    sCANMessage.ui32MsgID = 2; /* NODE 2 ID */
    sCANMessage.ui32MsgIDMask = 0;
    sCANMessage.ui32Flags = MSG_OBJ_TX_INT_ENABLE;
    sCANMessage.ui32MsgLen = sizeof(pui8MsgData);
    sCANMessage.pui8MsgData = pui8MsgData;
    ...
    while (1)
    {
        ...
        // CAN ID, which is also 1 in this example).  This function will cause
        // the message to be transmitted right away.
        if (FLAG_ONE_TX == 1)/* check if this flag == 1 so the receiver received succesfuly and you can transmit */
        {
            CANMessageSet(CAN0_BASE, 1, &sCANMessage, MSG_OBJ_TYPE_TX);
            FLAG_ONE_TX = 0;
        }
...
//*****************************************************************************
//
// Transmit Task for Node two
//
//*****************************************************************************
void vTaskCode_NODE_TWO_TX(void * pvParameters)
{
    tCANMsgObject sCANMessage;
    uint32_t ui32MsgData;
    uint8_t *pui8MsgData;/* pointer to check data */
    ...
    while (1)
    {
        ...
        // CAN ID, which is also 1 in this example).  This function will cause
        // the message to be transmitted right away.
        if (FLAG_TWO_TX == 1)/* check if this flag == 1 so the receiver received succesfuly and you can transmit */
        {
            CANMessageSet(CAN0_BASE, 1, &sCANMessage, MSG_OBJ_TYPE_TX);
            FLAG_TWO_TX = 0;
        }
...
//*****************************************************************************
//
// Receive task for Node TWO 2
//
//*****************************************************************************
void vTaskCode_NODE_TWO_RX(void * pvParameters)/*RECIVE TASK*/
{
    tCANMsgObject sCANMessage;
    uint8_t pui8MsgData[8];
//
...
//
    while (1)
    {
        ...
        if (g_bRXFlag2)/*check if CAN Receive msg on mailbox 2 g_bRXFlag2 set when msg received on mailbox 2   */
        {
            ...
            //
            CANMessageGet(CAN0_BASE, 2, &sCANMessage, 0);
            if (sCANMessage.ui32MsgID == 2) /* NODE 2 ID */
            {
                if (*sCANMessage.pui8MsgData == 1)/* LED ON */
                {
                    GPIOPinWrite(GPIO_PORTF_BASE, (GPIO_PIN_1), GPIO_PIN_1);/* LED ON */
...
//*****************************************************************************
//
// Startup code for node which initiate the network
//
//*****************************************************************************
