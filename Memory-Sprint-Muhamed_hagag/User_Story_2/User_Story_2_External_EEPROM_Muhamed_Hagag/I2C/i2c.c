...
{
    I2CSlaveEnable(I2C1_BASE); /* enable slave */
    I2CSlaveInit(I2C1_BASE, SLAVE_ADDR); /* INIT slave by slave address */
}
...
void mode(uint8_t SLAVE_ADDR, uint8_t mode)
{
    ...
    {
        I2CMasterSlaveAddrSet(I2C1_BASE, SLAVE_ADDRESS, true); /* read from slave */
    }

    else if (mode == WRITE)/* if mode == WRITE , = 0 that master write in slave */
    {
        I2CMasterSlaveAddrSet(I2C1_BASE, SLAVE_ADDRESS, false); /* write in slave */
    }
}
...
{
    uint32_t data = 0;
    data = I2CSlaveDataGet(I2C1_BASE); /* read data from slave */
...
/* O/p : N/A */
/* Return : Void */
/* Function that let master write in slave device   */
void I2c_master_Write(uint32_t Data, uint32_t Slave_address)
{
    I2CMasterSlaveAddrSet(I2C1_BASE, Slave_address, false); /* select slave address for device */
    I2CMasterDataPut(I2C1_BASE, Data); /* write data in slave device */
