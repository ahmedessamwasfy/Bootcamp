...
VAR(Dio_LevelType,memclass) Level)
{
    ...
    {
        for (index = 0U; index < (uint8) MAX_SIZE; index++)
        {
...
VAR(Dio_ChannelType,memclass) ChannelId)
{
    ...
    {
        for (index_ = 0U; (uint8) index_ < (uint8) MAX_SIZE; index_++)
        {
...
{
    uint8 index = (uint8) 0;
    uint8 final = 0;/* value which returned by Dio_WritePort serves */
...
P2CONST(Dio_ChannelGroupType,AUTOMATIC,PTR_FAR) ChannelGroupIdPtr, VAR(Dio_PortLevelType,AUTOMATIC) Level )
            {
                ...
                {
                    case PORTA_:
                    PORTA->DATA|=(channel_group->mask & Level);
                    break;
                    case PORTB_:
                    PORTB->DATA|=(channel_group->mask & Level);
                    break;
                    case PORTC_:
                    PORTC->DATA|=(channel_group->mask & Level);
                    break;
                    case PORTD_:
                    PORTD->DATA|=(channel_group->mask& Level);
                    break;
                    case PORTE_:
                    PORTE->DATA|=(channel_group->mask & Level);
                    break;
                    case PORTF_:
                    PORTF->DATA|=(channel_group->mask& Level);
                    break;
                }
            }
