...
void schadular_init_start()
{
	...
	sort_task();
	Remove(GO);
	/*the removed task is GO so i send the name of 
...
{
	uint8 i,j;
	for(i=ZERO ; i<NUMBER_OF_TASK ; ++i)/*loop for sort */
	{
...
{
	uint8 i ,num ;
	for(i=ZERO ; i<index ; i++)/* loop from 0 to index too get the index of removed item */
	{
